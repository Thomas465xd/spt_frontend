...
export default function CartTable({ cartDetails }: CartTableProps) {
	...
	const totalDiscount = cartDetails.reduce(
		(sum, item) => sum + item.cd_unit_value * discount * item.quantity,
		0,
	);
					...
					<table className="w-full min-w-[700px] divide-y divide-gray-300">
						...
						<tbody className="divide-y divide-gray-200 bg-white">
							{cartDetails.map((item: CartDetailData) => {
								const itemDiscount =
									item.cd_discount || item.discount || 0;
							...
							{cartDetails.length > 0 && (
								<>
									<tr className="bg-gray-50">
										<td
											colSpan={hasAnyDiscount ? 4 : 3}
											className="py-4 pl-4 pr-3 text-left sm:text-right font-medium text-gray-900 sm:pl-6"
										>
									...
									<tr className="bg-gray-50">
										<td
											colSpan={hasAnyDiscount ? 4 : 3}
											className="py-2 pl-4 pr-3 text-left sm:text-right font-medium text-gray-900 sm:pl-6"
										>
											IVA (19%):
										</td>
									...
									<tr className="bg-gray-100">
										<td
											colSpan={hasAnyDiscount ? 4 : 3}
											className="py-4 pl-4 pr-3 text-left sm:text-right font-semibold text-gray-900 sm:pl-6 text-base"
										>
											Total a pagar:
										</td>
