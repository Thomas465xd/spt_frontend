...
import { IdentificationsEnum, UserRegistrationForm } from "@/types/index";
...
export default function RegisterForm() {
	...
	const selectedIdType = watch("idType") as IdentificationsEnum;
	...
	const handleIdTypeChange = (newIdType: IdentificationsEnum) => {
		...
		>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
				<div className="space-y-2">
					...
					/>
					{errors.name && (
						<ErrorMessage mini>{errors.name.message}</ErrorMessage>
					)}
				</div>
				...
				<div className="space-y-2">
					...
					<input
						...
						{...register("businessName", {
							required:
								"El Nombre de la Empresa no puede ir vacío",
						})}
					/>
					{errors.businessName && (
						<ErrorMessage mini>
				...
				<div className="space-y-2">
					...
					<select
						...
						{...register("idType", {
							...
							onChange: (e) =>
								handleIdTypeChange(
									e.target.value as IdentificationsEnum,
								),
						})}
					>
						{ID_TYPES.map((type) => (
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
				<div className="space-y-2">
					<label htmlFor="personalId" className="label">
						{idLabels.personal}
					</label>
					<input
						type="text"
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
				<div className="space-y-2">
					...
					/>
					{errors.email && (
						<ErrorMessage mini>{errors.email.message}</ErrorMessage>
					)}
				</div>
				...
				<div className="space-y-2">
					...
					/>
					{errors.phone && (
						<ErrorMessage mini>{errors.phone.message}</ErrorMessage>
					)}
				</div>
			</div>
			...
			<div className="space-y-2 border-b border-gray-400 pb-8">
				...
				<input
					...
					className="input"
					{...register("address", {
						required: "Tu Dirección no puede ir vacía",
					})}
				/>
				{errors.address && (
					<ErrorMessage mini>{errors.address.message}</ErrorMessage>
				)}
			</div>

			<p className="text-sm text-gray-400">
				<span className="text-red-500">*</span> Estos datos serán
				enviados al equipo administrativo de SPT para la verificación de
				sus datos.
			</p>
